...
- Écrans : `#boutique` (objets pub : `totB()` calcule les sous-totaux par circuit `data-circ` et la jauge franco), `#dash` (frise 2027, KPI, bandeau J-12), `#cmd` (Rentrée des classes 2027 : kit obligatoire, PLV, questionnaire, total au fil de l'eau), `#perso` (Anniversaire 10 ans : champs à gauche, aperçu temps réel à droite, envoi BAT), `#projet` (Dépliant Noël : 5 jalons, relecture BAT v3, intervenants), `#admin` (Espace centrale, 7 onglets : Opérations, Fiches produits, Boutique, Gabarits, Utilisateurs, Magasins, Extracteur - création + liste sur chaque onglet).
...
- Gabarits historiques (a2/a5/vitro) : `ZONES_DEF` par format, `gabarit()` masque champs et zones d'aperçu non ouverts. Rendu fixe inchangé (`.creat`/`.zone` dans `#affiche`), ne pas régresser en le confondant avec le rendu des gabarits personnalisés (voir ci-dessous).

- Éditeur de gabarit personnalisé (Centrale > Gabarits) : état courant dans `gabState` (fond + `elements[]`, positions/tailles en % de `#gab-canvas`), rendu par `gabRenderCanvas()`, propriétés de l'élément sélectionné par `gabRenderProps()`, déplacement/redimensionnement par pointer events (`gabStartDrag`/`gabStartResize`/`gabOnDrag`, coordonnées en %, donc indépendant de la taille d'écran). `publierGabaritCustom()` fige une copie dans `GABARITS_CUSTOM` et ajoute une `<option data-custom="1" data-gid="...">` à `#f-gab` (même principe que l'ancien `creerGabarit()` pour le sélecteur `f-gab`). Un élément de type zone est soit lié à une donnée (`champ` parmi les mêmes clés que `ZONES_DEF` + `photo`), soit à un texte fixe (`champ:'libre'`, verrouillé - la centrale le saisit une fois, le magasin ne le modifie pas).
- Types d'éléments : `rect`/`cercle` (couleur, opacité, `bordureCouleur`/`bordureEpaisseur`, `radius` pour les rectangles), `zone` (texte/photo, voir ci-dessus), `prix` (élément spécialisé : `entier`, `centimes`, `devise`, `devisePosition` avant/après, `centimesStyle` normal/exposant - rendu par le helper partagé `gabPrixHTML()`, réutilisé par l'éditeur et par `renderGabaritCustom()` pour rester identique). La priorité d'affichage (lequel est dessus) suit simplement l'ordre du tableau `elements` (dernier = au-dessus) ; `gabDeplacerZid(id,dir)` (`'up'|'down'|'front'|'back'`) réordonne ce tableau depuis les boutons du panneau de propriétés ou de la ligne de calque - ne pas réintroduire un `z-index` CSS séparé, ce serait une deuxième source de vérité.
- Chaque élément porte aussi `nom` (affiché/éditable dans le panneau Calques et dans les propriétés), `visible` (masqué = grisé dans l'éditeur via `.gab-masque`, complètement ignoré par `renderGabaritCustom()`/`composerRenduCustom()`) et `verrou` (bloque `gabStartDrag`/`gabStartResize`, mais la sélection reste possible). Les groupes sont `gabState.groupes:[{id,nom}]` + le champ `groupe` sur chaque membre ; déplacer un membre (`gabOnDrag`) déplace tous les membres du même groupe par le même delta - le redimensionnement reste individuel (non groupé), simplification volontaire. La sélection multiple (`gabState.multiSel`, cases à cocher du panneau Calques) déclenche le panneau « Sélection multiple » (`gabRenderPropsMulti`) avec actions de masse (`gabActionMasse`) et alignement automatique (`gabAligner`) ; distinct de `gabState.selected` (sélection simple, clic direct) qui pilote le panneau de propriétés normal.
- Bibliothèque graphique par enseigne (`ENSEIGNE_CHARTE`, `GAB_BIBLIO`) : pas de fichiers image réels (aucun asset fourni pour ce prototype) - chaque entrée instancie un élément existant (`zone`, `rect` ou `prix`) avec des réglages prédéfinis à la couleur de l'enseigne. Ajouter une enseigne = une entrée dans `ENSEIGNE_CHARTE` ; ajouter un type d'élément de bibliothèque = une fonction dans `GAB_BIBLIO` (prend `ens,ch` et renvoie un objet élément partiel, complété par `gabAjouterDepuisBiblio()`).
- `gabarit()` détecte `dataset.custom==='1'` et bascule `#affiche` en mode `custom-active` (masque `.creat`/`.zone`, affiche `#affiche-custom`, aspect-ratio posé en JS selon `MM_FORMAT`) ; `renderGabaritCustom()` dessine le fond + les éléments avec les valeurs live (`champValeur()` lit les champs `f-*`). `maj()` et `photo()` rappellent `renderGabaritCustom()` si le gabarit actif est personnalisé, pour rester à jour en temps réel.
- Export (Personnaliser) : `composerRendu()` est le point d'entrée (async), dispatché vers `composerRenduFixe()` (gabarits historiques, code inchangé) ou `composerRenduCustom()` (dessine le fond + les éléments sur le canvas, échelle de police via `GAB_CANVAS_PX/GAB_REF_WIDTH`). PNG/JPEG : `telechargerImage()` (natif). PDF : `window.jspdf` (cdnjs, `defer`, mis en cache par le SW après la première visite), format réel en mm (`MM_FORMAT`) ; repli PNG si la librairie n'est pas encore en cache. `exportBAT()` est asynchrone (`await composerRendu()`).
...
- ✅ v1.12 : zone prix - troisième position pour la devise, « au milieu » (entre l'entier et les centimes, ex. 19€90), en plus d'avant/après. Le séparateur virgule est masqué dans ce mode (la devise joue déjà ce rôle visuel). Répercuté dans `gabPrixHTML()` (éditeur + aperçu Personnaliser) et dans le dessin canvas de `composerRenduCustom()`. SW `pm-proto-v12`.
- ✅ v1.11 : éditeur de gabarit - panneau de calques et bibliothèque graphique par enseigne, interface à 3 colonnes façon Canva (outils | canvas | propriétés). Panneau Calques : liste des éléments (icône, nom, visibilité, verrou, avancer/reculer), renommage en ligne (double-clic), groupes (glisser un membre déplace tout le groupe), sélection multiple par cases à cocher avec actions de masse (dupliquer, supprimer, masquer/afficher, verrouiller/déverrouiller, premier/arrière-plan, grouper) et alignement automatique (gauche/centre/droite, haut/milieu/bas, distribution horizontale). Bibliothèque : éléments prêts à l'emploi à la charte de chaque enseigne (Sport 2000, Sport 2000 Montagne, Mondovelo, S2 Sneakers) - logo texte, bandeau, étiquette prix, cadre décoratif - insérés en un clic (pas de fichiers image réels dans ce prototype : la charte est portée par la couleur/typo, réutilisant les types d'éléments existants). Les calques masqués sont ignorés à l'aperçu Personnaliser et à l'export ; verrouillés = non déplaçables/redimensionnables. SW `pm-proto-v11`.
- ✅ v1.10 : éditeur de gabarit enrichi - édition des formes (bordure couleur/épaisseur, arrondi des angles pour les rectangles), priorité d'affichage des blocs (avancer/reculer/premier plan/arrière-plan, réordonne le tableau `elements` qui fait aussi foi pour l'empilement visuel), nouveau type d'élément « Zone prix » spécialisé (partie entière, centimes, symbole devise personnalisable et sa position avant/après, centimes en affichage Normal (19,90 €) ou Exposant (19 € 90, à la façon d'une étiquette prix)). Le tout est répercuté à l'identique dans l'éditeur, l'aperçu Personnaliser et l'export PNG/JPEG/PDF (même modèle de données, trois moteurs de rendu synchronisés). SW `pm-proto-v10`.
- ✅ v1.9 : éditeur visuel de gabarit (onglet Centrale > Gabarits d'impression locale), intégré de bout en bout dans Personnaliser et dans l'export. Toujours 100 % HTML/JS statique : la centrale charge un fond (image), pose des rectangles, des cercles et des zones (texte lié à une donnée de personnalisation - remise, date, adresse, horaires, réseaux, accroche - ou texte libre fixe, ou photo locale), positionne/redimensionne chaque élément par glisser-déposer (pointer events, positions en % pour rester responsive), règle police/taille/couleur/alignement/gras par zone. À la publication, le gabarit apparaît dans le sélecteur de Personnaliser : l'aperçu (`#affiche-custom`) et l'export (PNG/JPEG/PDF) sont recalculés dynamiquement à partir du même modèle (fond + éléments), avec les valeurs saisies par le magasin en temps réel. Les 3 gabarits historiques (Affiche événement A2, Flyer promo A5, Vitrophanie) restent inchangés, sur le rendu fixe existant - le nouvel éditeur ne s'applique qu'aux gabarits créés à partir de maintenant, cohérent avec le principe déjà posé (reprise des ~200 gabarits existants à part). SW `pm-proto-v9`.
- ✅ v1.8 : Espace centrale étendu à 7 onglets (au lieu de 3) : Opérations (création d'une opération nationale/locale, alimente les listes « Opération » de Fiches produits et de l'Extracteur), Fiches produits (inchangé), Boutique (formulaire « Nouvel objet publicitaire » déplacé ici depuis Fiches produits, dans son propre onglet), Gabarits (inchangé), Utilisateurs (création + liste, 3 rôles : magasin adhérent / centrale / agence externe, rattachement à un magasin), Magasins (création + liste, pré-alimentée par les 8 magasins déjà utilisés par l'extracteur), Extracteur (inchangé). Chaque nouvel onglet suit le même principe que l'existant : création + liste (`liste-admin`), sans modification/suppression - cohérent avec le fait que ces écrans ne sont pas contractuels. La création d'un magasin alimente aussi le sélecteur « Magasin de rattachement » de l'onglet Utilisateurs. SW `pm-proto-v8`.
