...
- Types d'éléments : `rect`/`cercle` (couleur, opacité, `bordureCouleur`/`bordureEpaisseur`, `radius` pour les rectangles), `zone` (texte/photo, voir ci-dessus), `prix` (élément spécialisé : `entier`, `centimes`, `devise`, `devisePosition` avant/après, `centimesStyle` normal/exposant - rendu par le helper partagé `gabPrixHTML()`, réutilisé par l'éditeur et par `renderGabaritCustom()` pour rester identique). La priorité d'affichage (lequel est dessus) suit simplement l'ordre du tableau `elements` (dernier = au-dessus) ; `gabDeplacerZ('up'|'down'|'front'|'back')` réordonne ce tableau depuis les boutons du panneau de propriétés - ne pas réintroduire un `z-index` CSS séparé, ce serait une deuxième source de vérité.
